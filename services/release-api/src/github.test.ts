...
describe('GitHubClient', () => {
  it('uses the TTL/ETag cache for repeated release reads', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith('https://api.github.com/repos/udiram/VoxelWeave-Designer/releases/latest', expect.objectContaining({ headers: expect.objectContaining({ Accept: 'application/vnd.github+json' }) }));
  });
