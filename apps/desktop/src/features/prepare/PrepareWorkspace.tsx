...
export function PrepareWorkspace() {
  ...
  const geometryReady = state.source.path ? state.selection.created : hasModeledSolid;
  ...
  const checks = [
    ["Geometry", geometryReady, state.selection.created ? "DICOM selection and visible regions present" : hasModeledSolid ? "Modeled solid ready for canonical validation" : "Create a selection or modeled solid"],
    ["Calibration", state.calibrations.some((profile) => profile.accepted), "Accepted profiles available"],
    ["Flow limits", geometryReady, geometryReady ? "Checked by sidecar on generation" : "Pending geometry"],
    ["Bed bounds", geometryReady, geometryReady ? "Checked by sidecar on generation" : "Pending geometry"],
  ...
  return <div className="workspace-layout prepare-layout">
    ...
    <aside className="right-inspector prepare-inspector" aria-label="Prepare run checks inspector">
      ...
      <Disclosure title="Estimate" open={estimateOpen} onToggle={() => setEstimateOpen((open) => !open)}><div className="estimate-table"><div><span>Print time</span><strong>{state.toolpath.estimated.printTime}</strong></div><div><span>T0 (Natural PLA)</span><strong>{formatMass(state.toolpath.estimated.t0Grams)}</strong></div><div><span>T1 (White PLA)</span><strong>{formatMass(state.toolpath.estimated.t1Grams)}</strong></div><div><span>Tool changes</span><strong>{state.toolpath.estimated.toolChanges}</strong></div></div>{state.toolpath.estimated.massStatus && <p className="estimate-status">Mass estimate · {state.toolpath.estimated.massStatus}</p>}</Disclosure>
...
function formatMass(value: number | null): string {
  return value === null || !Number.isFinite(value) ? "Unavailable" : `${value.toFixed(1)} g`;
}
