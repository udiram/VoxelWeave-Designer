...
export function DicomWorkspace() {
  ...
  const orientationSpacing = selection.orientation === "axial" ? state.source.spacing.z : selection.orientation === "sagittal" ? state.source.spacing.x : state.source.spacing.y;
  ...
  }, [dispatch, sidecar, state.source.path, state.source.sliceCount, state.source.cache.directory, state.source.sourceHash, state.source.directionLps, selection.start, selection.orientation, selection.resamplingMethod]);
  ...
  return <div className="workspace-layout dicom-layout">
    ...
    <aside className="right-inspector dicom-inspector" aria-label="Print selection inspector">
      ...
      <div className="inspector-section">
        <h3>Selection</h3>
        <SegmentedControl label="Selection type" value={selection.kind} options={[{ value: "single", label: "Single slice" }, { value: "range", label: "Inclusive range" }, { value: "tiles", label: "Tile range" }]} onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { kind: value as SelectionKind, outputMode: value === "tiles" ? "tiles" : "continuous" } })} />
        {selection.kind !== "single" ? <div className="range-fields">
          <NumberField label="Start index" value={selection.start} min={0} max={maxIndex} onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { start: value } })} />
          <NumberField label="End index" value={selection.end} min={selection.start} max={maxIndex} onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { end: value } })} />
          <div className="position-readout"><span>Start physical position</span><strong>{positionForIndex(selection.start).toFixed(2)} mm</strong><span>End physical position</span><strong>{positionForIndex(selection.end).toFixed(2)} mm</strong></div>
          <div className="thickness-readout"><strong>{selection.thicknessMm.toFixed(2)} mm</strong><span>physical slab · {Math.max(1, selection.end - selection.start + 1)} planes</span></div>
        </div> : <div className="range-fields">
          <div className="single-note">Source plane repeated through an explicit physical print thickness.</div>
          <NumberField label="Print thickness" value={selection.singleThicknessMm ?? orientationSpacing} min={0.01} step={0.01} suffix="mm" onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { singleThicknessMm: value } })} />
        </div>}
        {selection.kind === "tiles" && <div className="tile-controls">
          <NumberField label="Tile thickness" value={selection.tileThicknessMm ?? selection.thicknessMm} min={0.01} step={0.01} suffix="mm" onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { tileThicknessMm: value } })} />
          <NumberField label="Tile stride" value={selection.stride} min={1} max={Math.max(1, selection.end - selection.start + 1)} onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { stride: value } })} />
          <NumberField label="Plate columns" value={selection.tilePlateColumns ?? 4} min={1} max={12} onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { tilePlateColumns: value } })} />
          <NumberField label="Plate rows" value={selection.tilePlateRows ?? 1} min={1} max={12} onChange={(value) => dispatch({ type: "SET_SELECTION", patch: { tilePlateRows: value } })} />
          <label className="checkbox-field"><input type="checkbox" checked={selection.tileOrientationMarkers ?? true} onChange={(event) => dispatch({ type: "SET_SELECTION", patch: { tileOrientationMarkers: event.target.checked } })} /> Orientation markers</label>
          <label className="checkbox-field"><input type="checkbox" checked={selection.tileTabs ?? false} onChange={(event) => dispatch({ type: "SET_SELECTION", patch: { tileTabs: event.target.checked } })} /> Structural tabs</label>
        </div>}
      </div>
